...
static int decode_interrupt_cb(void *ctx)
{
    ...
    if (is->ioOperationPreTime != 0 && is->isForceTimeout && av_gettime() - is->ioOperationPreTime >= is->readTimeOutUs) {
        av_log(NULL, AV_LOG_ERROR, "ffmpeg operation timeout!");
        isTimeout = true;
    }
    return is->abort_request || (isTimeout && is->isForceTimeout);
}
...
static int read_thread(void *arg)
{
    ...
    if(dictionary != NULL && ffp->format_opts != NULL) {
        AVDictionaryEntry * entry = av_dict_get(ffp->format_opts, "read_timeout", NULL, AV_DICT_MATCH_CASE);
        if(entry != NULL) {
            int timeout = atoi(entry->value);
            is->isForceTimeout = 1;
            is->readTimeOutUs = timeout;
        } else {
            is->isForceTimeout = 0;
        }
    }
    ...
    if(av_stristart(is->filename, "rtsp", NULL)) {
        ...
        if(entry != NULL) {
            av_dict_set(&ffp->format_opts, entry->key, entry->value, AV_DICT_DONT_OVERWRITE);
            av_log(ffp, AV_LOG_WARNING, "SET rtsp_transport %s \n",entry->value);
        }
       
    }
    
    if (av_stristart(ic->filename, "rtsp://192.168.1.254/",NULL) ) {
        av_log(ffp, AV_LOG_WARNING, "rtsp://192.168.1.254/ SET rtsp_transport UDP");
        av_dict_set(&ffp->format_opts, "rtsp_transport", "udp", AV_DICT_DONT_OVERWRITE);
    }
...
//      of the seek_pos/seek_rel variables
        ...
        is->ioOperationPreTime = av_gettime();
